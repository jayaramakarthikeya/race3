...
#   plot_path(
#     os.path.join(plot_dir, "path_{}_{}.".format(trajectory.vehicle.mass, trajectory.vehicle.cof) + args.ext),
...
if args.plot_trajectory or args.plot_all:
  plot_trajectory(
    os.path.join(plot_dir, "trajectory_{}_{}.".format(map_id, trajectory.vehicle.cof) + args.ext),
    track.left, track.right, trajectory.path.position(trajectory.s),
    trajectory.velocity.v
  )

# Save trajectory
with open(os.path.join(plot_dir, "{}_{}.pickle".format(map_id, trajectory.vehicle.cof)), 'wb') as f:
  pickle.dump(trajectory, f)

traj_time = np.diff(trajectory.s) / trajectory.velocity.v
cum_time = np.cumsum(traj_time)

plt.figure()
plt.plot(cum_time, trajectory.velocity.v_local, linestyle='-.')
plt.plot(cum_time, trajectory.velocity.v_acclim, linestyle='--')
plt.plot(cum_time, trajectory.velocity.v_declim, linestyle=':')
plt.plot(cum_time, trajectory.velocity.v, linestyle='-')
# plt.legend(['v', 'v_acclim', 'v_declim'])
plt.xlabel('t')
plt.ylabel('v')
plt.ylim(0, 10)
plt.title('Velocity Profile')
plt.savefig(os.path.join(plot_dir, 'velocity_t_{}_{}.{}'.format(trajectory.vehicle.mass, trajectory.vehicle.cof, args.ext)))


plt.figure()
plt.plot(trajectory.s[:-1], trajectory.velocity.v_local, linestyle='-.')
plt.plot(trajectory.s[:-1], trajectory.velocity.v_acclim, linestyle='--')
plt.plot(trajectory.s[:-1], trajectory.velocity.v_declim, linestyle=':')
plt.plot(trajectory.s[:-1], trajectory.velocity.v, linestyle='-')
# plt.legend(['v', 'v_acclim', 'v_declim'])
plt.xlabel('s')
plt.ylabel('v')
plt.ylim(0, 10)
plt.title('Velocity Profile')
plt.savefig(os.path.join(plot_dir, 'velocity_s_{}_{}.{}'.format(trajectory.vehicle.mass, trajectory.vehicle.cof, args.ext)))
